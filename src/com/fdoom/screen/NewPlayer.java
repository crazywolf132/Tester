...
public class NewPlayer extends Menu
{
    ...
    static {
    	NewPlayer.loadworld = false;
    	NewPlayer.worldname = "";
        options = new String[] { "Load World", "New World" };
    }
    
    public NewPlayer(final Menu parent) {
        this.selected = 0;
    ...
    public void tick() {
        ...
        if (this.createworld) {
            ...
            if (this.input.enter.clicked && this.wncol == Color.get(0, 5, 5, 5)) {
            	NewPlayer.worldname = this.name;
            	OptionFile.name = this.name;
            	OptionFile.writeOpt();
                this.name = "";
               // this.game.setMenu(new ModeMenu());
            }
        }
        if (NewPlayer.loadworld && this.input.menu.clicked && !this.rename) {
            if (!this.delete) {
            	NewPlayer.worldname = this.worldnames.get(this.worldselected);
                Sound.test.play();
               // this.game.resetstartGame();
                ...
                NewPlayer.loadworld = false;
                if (this.worldnames.size() > 0) {
                    this.game.setMenu(new NewPlayer(this.parent));
                }
                else {
                    this.game.setMenu(new TitleMenu());
                }
        ...
        if (this.rename) {
            ...
            if (this.input.attack.clicked && this.wncol == Color.get(0, 5, 5, 5)) {
                ...
                this.name = "";
                final File world = new File(String.valueOf(this.location) + "/" + this.worldnames.get(this.worldselected));
                world.renameTo(new File(String.valueOf(this.location) + "/" + NewPlayer.worldname));
                this.game.setMenu(new NewPlayer(this.parent));
                this.tick = 0;
                this.rename = false;
            }
        ...
        if (!this.createworld && !NewPlayer.loadworld) {
            if (this.input.menu.clicked) {
                System.out.println(this.selected);
                if (this.selected == 0) {  
                	NewPlayer.loadworld = true;
                    this.createworld = false;
                }
                if (this.selected == 1) {
                    this.name = "";
    ...
    public void render(final Screen screen) {
        ...
        if (!this.createworld && !NewPlayer.loadworld) {
            for (int lo = 2, i = 0; i < lo; ++i) {
                if (this.fw || i != 0) {
                    String msg = NewPlayer.options[i];
        ...
        else if (this.createworld && !NewPlayer.loadworld) {
            final String msg2 = "Your username";
