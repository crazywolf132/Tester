...
public class WorldSelectMenuTest extends Menu
{
    private Menu parent;
    ...
    static {
        WorldSelectMenuTest.loadworld = false;
        WorldSelectMenuTest.worldname = "";
        options = new String[] { "Load World", "New World" };
    }
    
    public WorldSelectMenuTest(final Menu parent) {
        this.selected = 0;
    ...
    public void tick() {
        ...
        if (WorldSelectMenuTest.loadworld) {
            if (this.input.up.clicked) {
                --this.worldselected;
            }
            if (this.input.down.clicked) {
        ...
        if (this.createworld) {
            ...
            if (this.input.enter.clicked && this.wncol == Color.get(0, 5, 5, 5)) {
                WorldSelectMenuTest.worldname = this.name;
        ...
        if (WorldSelectMenuTest.loadworld && this.input.menu.clicked && !this.rename) {
            if (!this.delete) {
               ...
               // this.game.resetstartGame();
                ...
                WorldSelectMenuTest.loadworld = false;
                if (this.worldnames.size() > 0) {
                    this.game.setMenu(new WorldSelectMenuTest(this.parent));
                }
                else {
                    this.game.setMenu(new TitleMenu());
                }
        ...
        if (this.rename) {
            ...
            if (this.input.attack.clicked && this.wncol == Color.get(0, 5, 5, 5)) {
                ...
                this.name = "";
                final File world = new File(String.valueOf(this.location) + "/" + this.worldnames.get(this.worldselected));
                world.renameTo(new File(String.valueOf(this.location) + "/" + WorldSelectMenuTest.worldname));
                this.game.setMenu(new WorldSelectMenuTest(this.parent));
                this.tick = 0;
                this.rename = false;
            }
        ...
        if (!this.createworld && !WorldSelectMenuTest.loadworld) {
            if (this.input.menu.clicked) {
                System.out.println(this.selected);
                if (this.selected == 0) {  
                    WorldSelectMenuTest.loadworld = true;
                    this.createworld = false;
                }
                if (this.selected == 1) {
                    this.name = "";
    ...
    public void render(final Screen screen) {
        ...
        if (!this.createworld && !WorldSelectMenuTest.loadworld) {
            for (int lo = 2, i = 0; i < lo; ++i) {
                if (this.fw || i != 0) {
                    String msg = WorldSelectMenuTest.options[i];
        ...
        else if (this.createworld && !WorldSelectMenuTest.loadworld) {
